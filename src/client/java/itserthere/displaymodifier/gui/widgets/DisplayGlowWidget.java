...
import itserthere.displaymodifier.gui.DisplayGlowScreen;
...
public class DisplayGlowWidget extends ObjectSelectionList<DisplayGlowWidget.ListEntry> {

    private final DisplayGlowScreen parent;
    private final int listWidth;
    private final Component title;

    public DisplayGlowWidget(DisplayGlowScreen parent, Component title, int listWidth, int top, int bottom) {
    ...
    public class ListEntry extends Entry<ListEntry> {
        ...
        ListEntry(Display display, DisplayGlowScreen parent) {
        ...
        public void render(GuiGraphics guiGraphics, int entryIdx, int top, int left, int entryWidth, int entryHeight,
            ...
            renderDisplay(guiGraphics, left + 16, top + 28, 15);
        }
        public Display getDisplay() {return display;}
        public void renderDisplay(GuiGraphics guiGraphics, int xPos, int yPos, int size) {
            if (display != null) {
                DisplayData displayData=new DisplayData();
                displayData.readFromEntity(display);
                InventoryScreen.renderEntityInInventory(guiGraphics, xPos, yPos, size,
                        displayData.getTranslation(),
                        displayData.getRotation(),
                        (Quaternionf) null, display.getControllingPassenger());
            }
        }
        ...
        public boolean mouseClicked(double mouseX, double mouseY, int button) {
            ...
            DisplayGlowWidget.this.setSelected(this);
            return false;
        }

        public Component getPositionComponent() {
            return Component.literal(getDisplay().blockPosition().toShortString());
        }
